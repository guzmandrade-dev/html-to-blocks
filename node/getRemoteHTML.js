...
async function fetchFragment({ url, language = null, selector = "body" }) {
    ...
    if (language) {
        try {
            const langLink = await page
                .locator(`a[data-lang=${language}]`)
                .getAttribute("href")
                .catch(() => null);
            if (langLink) {
                const base = new URL(page.url());
                await page.goto(base.origin + langLink, {
    ...
    await page.evaluate((sel) => {
        ...
        function applyRecursive(el) {
            ...
            if (Object.keys(styles).length) {
                const merged = Object.entries(styles)
                    .map(([k, v]) => `${k}: ${v}`)
                    .join("; ");
...
if (require.main === module) {
    (async () => {
        ...
        if (!url) {
            console.error(
                "Usage: node getRemoteHTML.js --url=URL [--language=LANG] [--selector=CSS]",
            );
            process.exit(1);
        }
        try {
