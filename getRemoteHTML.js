...
/**
 * Runs the function to launch a Puppeteer browser, open a new page,
...
async function run() {
	...
	if (process.argv[3]) {
		let newLangURL = await page.$eval(
			"ul.lang-picker__languages li a[data-lang=" + process.argv[3] + "]",
	...
	if ( ! bodyError) {
		...
		if ( ! backToHome || process.argv[3] === undefined) {
			...
			console.log(
				source.replace(
					"</body>",
					'<span data-origin-url="' + page.url() + '" /></body>'
				)
			);
		}
	}
 ...
 *
 * @link https://stackoverflow.com/questions/51529332/puppeteer-scroll-down-until-you-cant-anymore#53527984
 *
